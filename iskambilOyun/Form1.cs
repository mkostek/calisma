...
namespace iskambilOyun
{
	public partial class Form1 : Form
	{
		...
		#endregion
		#region ne_kartı
		public string karttip(int atilan)
		{
			string dönder="";
					...
					if(elseri(p))//yere atılan ilk kart tipinden elinde varsa yerde koz kartından yoksa
					{
					   	if(!kozyerde())
							r=max(p);
					   		...
					   		r=elseri(p);//elinde yere atılan seride kart varsa
		...
		public bool elseri(int i)//parametre olarak aldığı kart numarasına göre eldeseri olup olmadığını kontrol eder 
		{
			...
			{
				if(karttip((int)atilan[0])==karttip((int)oyunEl[j]))
				{
					t=true;break;
				}
			}
		...
		#endregion		
		#region yer_kontrolu
		public bool yer(ArrayList ls,ArrayList yr)
		{
			for(int u=0;u<yr.Count;u++)
			{
			...
			return true;//yerde en büyük olan kartı döndürür ayrıca kazanını belirtir
		}
		#endregion
		#region alma_kontrolu
		public bool alma(ArrayList ls,ArrayList yr)
		{
			return true;
		}
		#endregion
		#region kart_kontrolu
		public bool atma(ArrayList ls,ArrayList s,int k)
		{	//birinci parametre kart atacak olanı,ikinci parametre yeri belirtir,
			/*	//döndürdüğü değer atılacak olan kartı tanımlar
		int a =(int)s[0];
		if(0=<a && a<13)
		{
			for(int i=0;i<ls.Count;i++)
			{
				if(a<ls[i] || ls[i]==0 && 0=<ls[i] && ls )
				{
					return false;
				}
			}
		...
		//		if(eldekoz((int)oyunEl[kartNo])==true)MessageBox.Show("yerde");
						...
						pcb.Location = new Point(475, 250);//ortaya kart  at
						pcb.BringToFront();

...
namespace iskambilOyun
{
	public partial class Form1 : Form
	{
		...
		#endregion
		#region eldesuy
		public bool eldesuy(int i)//parametre olarak aldığı kart numarasına göre eldeseri olup olmadığını kontrol eder
		{
			bool t=false;
			if(attı==karttip((int)oyunEl[i]))
				t=true;
		...
		public void kapat()
		{
			...
			foreach (Control c in this.Controls)
			{
				...
				{
					//MessageBox.Show();
					if(int.Parse(((PictureBox)c).Name)>= a[0] && int.Parse(((PictureBox)c).Name) < a[1])
					c.Show();
				else c.Hide();	
				}
